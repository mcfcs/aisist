(function (root) {
  ...
  function courseBlock(entry, { picks, onToggle, searching }) {
    ...
    head.append(el('b', entry.course.code));
    ...
    if (!entry.offerings.length) {
      holder.append(el('p', searching ? 'Looking for sections…' : 'No section found for this term yet.', 'suggest-empty'));
      return holder;
    }
  ...
  function suggestionPanel({ remaining, sections, term, picks = [], onToggle, searching = false }) {
    ...
    if (groups.due.length) {
      for (const entry of groups.due) wrap.append(courseBlock(entry, { picks, onToggle, searching }));
    } else {
    ...
    if (groups.other.length) {
      ...
      for (const entry of groups.other) other.append(courseBlock(entry, { picks, onToggle, searching }));
      wrap.append(other);
    }
    return wrap;
  }
  ...
  const api = { styles, el, colorFor, grid, pickTable, summary, clashingKeys, clashesWith, offerRow, suggestionPanel, SLOT, SLOT_HEIGHT };
