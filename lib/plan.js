(function (root) {
  ...
  // Only the hours a draft actually uses are drawn. A grid padded out to a
  // fixed 7am-to-7pm window makes a five-hour schedule unreadable.
  function gridBounds(picks) {
    const meetings = (picks || []).flatMap(pick => meetingsOf(pick));
  ...
  // A course the student must take that AISIS is not offering this term still
  // needs a place in the week, so a planned block stands in for a section.
  const PLANNED = 'PLANNED';
  const clockText = minutes => `${String(Math.floor(minutes / 60)).padStart(2, '0')}${String(minutes % 60).padStart(2, '0')}`;
  function customSection({ course, title = '', section = '', units = '', days = [], start, end, room = '', instructors = '' }) {
    const chosen = DAYS.filter(day => days.includes(day));
    if (!clean(course)) throw new Error('Enter a course code.');
    if (!chosen.length) throw new Error('Choose at least one day.');
    if (!Number.isFinite(start) || !Number.isFinite(end) || end <= start) throw new Error('Enter a start time earlier than the end time.');
    return {
      course: clean(course), section: clean(section) || 'PLAN', title: clean(title),
      units: units === '' || units === null ? '' : String(units),
      time: `${chosen.join('-')} ${clockText(start)}-${clockText(end)}(${PLANNED})`,
      room: clean(room), instructors: clean(instructors),
      maxNo: '', lang: '', level: '', freeSlots: '', remarks: '', dept: '', custom: true
    };
  }
  const isCustom = section => section?.custom === true;
  // A title is only carried by offered sections, so a planned block or a
  // program entry borrows one from whatever section of the course is known.
  function titleFor(course, sections) {
    const key = courseKey(typeof course === 'string' ? course : course?.code || course?.course);
    return clean((sections || []).find(section => courseKey(section.course) === key && clean(section.title))?.title);
  }
  ...
  function exportText(draft, term) {
    ...
    for (const pick of picks) {
      lines.push([pick.course, clean(pick.title), pick.section, `${pick.units || 0} units`, meetingLabel(pick) || 'TBA', clean(pick.room) || 'TBA', clean(pick.instructors), isCustom(pick) ? 'planned block, not an AISIS section' : ''].filter(Boolean).join('\t'));
    }
  ...
  const api = {
    ...
    PLANNED, clockText, customSection, isCustom, titleFor,
