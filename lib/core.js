(function (root) {
  ...
  function readRow(row, indices = { course: 0, section: 1, title: 2, professor: 6 }) {
    ...
    const data = { course, section: get('section'), title: get('title'), professors };
    // Callers that locate optional schedule columns, such as Time or Free
    // Slots, receive them alongside the fields every caller needs.
    for (const key of Object.keys(indices)) {
      if (!Object.hasOwn(data, key) && key !== 'professor' && indices[key] >= 0) data[key] = get(key);
    }
    return data;
  }
