...
- **Planned time blocks:** a course your program needs that AISIS is not offering this term can still be given a place in the week. **Add a planned time** beside such a course, or **Add a planned time block** under the weekly view, opens a short form for days, start, end, and a note. Planned blocks count toward units and conflicts like any other pick, and are marked as planned wherever they appear so they are never mistaken for a real section.
- **Save as image:** the weekly view exports as a PNG with course names, times, rooms, and conflicts drawn in, for sharing or printing.
...
The `storage` permission supports these caches, your drafts, and the feature toggles. Host permissions are `https://aisis.ateneo.edu/*`, which lets the planner tab read your program of study and class schedules with your existing AISIS session, and `https://profstopick.com/*` for professor profiles.
